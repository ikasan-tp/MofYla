...
function renderProducts(){
  ...
  const content = activeProductTab === 'wholesale'
    ? stores.map(store => {
        ...
        return `<section class="brand-wholesale-group"><div class="brand-mini-head"><h3>${escapeHtml(store)}</h3><span class="brand-chip ok">${items.length}商品 / 累計卸し${stock}</span></div><div class="brand-product-grid">${items.map(productCard).join('')}</div></section>`;
  ...
  root.innerHTML = `${pageHead('商品管理','卸し商品とネット販売在庫を分けて管理します。', '<button class="btn btn-primary" data-action="new-product">追加</button>')}${tabs}${summary}${content}`;
}

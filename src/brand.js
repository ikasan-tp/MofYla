...
function productKind(product = {}){
  return product.salesChannel || product.productMode || product.kind || (product.isWholesale ? 'wholesale' : 'online');
}
function productStore(product = {}){
  return product.storeName || product.shopName || product.wholesaleStore || product.clientName || '店舗未設定';
}
function productNumber(value){
  return Number(value || 0);
}
function moneyText(value){
  return `${productNumber(value).toLocaleString('ja-JP')}円`;
}
function productTabButton(kind, label, count){
  return `<button class="brand-filter ${activeProductTab === kind ? 'active' : ''}" type="button" data-action="set-product-tab" data-value="${kind}">${label}<span>${count}</span></button>`;
}
function renderProductCard(product){
  const kind = productKind(product);
  const stock = kind === 'wholesale' ? productNumber(product.wholesaleStock || product.stock) : productNumber(product.stock);
  const price = kind === 'wholesale' ? productNumber(product.wholesalePrice || product.price) : productNumber(product.price);
  return `<article class="brand-card brand-product-card">
    <div class="brand-product-head">
      <div class="brand-product-title">
        <h3>${escapeHtml(product.name || '商品名未設定')}</h3>
        <p>${escapeHtml(product.category || 'カテゴリ未設定')} / ${escapeHtml(product.status || '未設定')}</p>
    ...
    ${kind === 'wholesale' ? `<div class="brand-wholesale-store">${escapeHtml(productStore(product))}</div>` : ''}
    <div class="brand-product-metrics">
      <span><b>${moneyText(price)}</b><small>${kind === 'wholesale' ? '卸し価格' : '価格'}</small></span>
      <span><b>${moneyText(product.cost)}</b><small>原価</small></span>
      <span><b>${stock}</b><small>${kind === 'wholesale' ? '店舗在庫' : '在庫'}</small></span>
      <span><b>${productNumber(product.sold)}</b><small>販売数</small></span>
    </div>
    ${product.description ? `<p class="brand-note">${escapeHtml(product.description)}</p>` : '<p class="brand-note">説明はまだありません。</p>'}
    ${kind === 'wholesale' && product.wholesaleMemo ? `<p class="brand-note">${escapeHtml(product.wholesaleMemo)}</p>` : ''}
  </article>`;
}
function renderProducts(){
  const root = document.getElementById('brandProducts');
  if(!root) return;
  const onlineProducts = state.products.filter(product => productKind(product) !== 'wholesale');
  const wholesaleProducts = state.products.filter(product => productKind(product) === 'wholesale');
  const currentProducts = activeProductTab === 'wholesale' ? wholesaleProducts : onlineProducts;
  const totalStock = currentProducts.reduce((sum, product) => sum + productNumber(activeProductTab === 'wholesale' ? (product.wholesaleStock || product.stock) : product.stock), 0);
  const totalValue = currentProducts.reduce((sum, product) => {
    const stock = productNumber(activeProductTab === 'wholesale' ? (product.wholesaleStock || product.stock) : product.stock);
    const price = productNumber(activeProductTab === 'wholesale' ? (product.wholesalePrice || product.price) : product.price);
    return sum + stock * price;
  }, 0);
  const stores = [...new Set(wholesaleProducts.map(productStore))].sort((a, b) => a.localeCompare(b, 'ja'));
  const header = activeProductTab === 'wholesale'
    ? pageHead('商品管理', '卸し商品を店舗ごとに分けて管理します。', '<button class="btn btn-primary" data-action="new-product">卸し商品追加</button>')
    : pageHead('商品管理', 'ネット販売在庫と通常の商品情報を管理します。', '<button class="btn btn-primary" data-action="new-product">商品追加</button>');
  const summary = `<div class="brand-status-summary">
    ...
    <div class="brand-status-tile"><strong>${moneyText(totalValue)}</strong><span>在庫金額目安</span></div>
  </div>`;
  const tabs = `<div class="brand-product-tabs">
    ${productTabButton('online', 'ネット販売在庫', onlineProducts.length)}
    ${productTabButton('wholesale', '卸し商品', wholesaleProducts.length)}
  </div>`;
  if(activeProductTab !== 'wholesale'){
    root.innerHTML = `${header}${tabs}${summary}<div class="brand-product-grid">${onlineProducts.map(renderProductCard).join('') || empty('ネット販売在庫はまだありません。')}</div>`;
    return;
  }
  const groups = stores.map(store => {
    const items = wholesaleProducts.filter(product => productStore(product) === store);
    const stock = items.reduce((sum, product) => sum + productNumber(product.wholesaleStock || product.stock), 0);
    return `<section class="brand-wholesale-group">
      <div class="brand-mini-head"><h3>${escapeHtml(store)}</h3><span class="brand-chip ok">${items.length}商品 / 在庫${stock}</span></div>
      <div class="brand-product-grid">${items.map(renderProductCard).join('')}</div>
    </section>`;
  }).join('');
  root.innerHTML = `${header}${tabs}${summary}${groups || empty('卸し商品はまだありません。商品追加から店舗名を入れて登録できます。')}`;
}
function productForm(product = {}){
  const currentKind = productKind(product) === 'wholesale' ? 'wholesale' : activeProductTab;
  const values = { salesChannel:currentKind, ...product };
  openForm(product.id ? '商品編集' : (currentKind === 'wholesale' ? '卸し商品追加' : '商品追加'), [
    ...
    {name:'storeName',label:'店舗名（卸し商品の場合）'},
    {name:'price',label:'ネット販売価格',type:'number'},
    {name:'wholesalePrice',label:'卸し価格',type:'number'},
    {name:'cost',label:'原価',type:'number'},
    {name:'stock',label:'ネット販売在庫数',type:'number'},
    {name:'wholesaleStock',label:'店舗在庫数',type:'number'},
    ...
    upsert('products', {
      ...
      wholesaleStock:Number(data.wholesaleStock || 0),
...
document.addEventListener('click', event => {
  const el = event.target.closest('[data-action="set-product-tab"]');
  if(!el) return;
  activeProductTab = el.dataset.value === 'wholesale' ? 'wholesale' : 'online';
  renderProducts();
});

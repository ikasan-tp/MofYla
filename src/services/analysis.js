...
export function analyzeActivities(activities = []){
  ...
  const replies = activities.filter(a => a.type === 'reply').length;
  ...
  if(replies === 0 && activities.length >= 4) notes.push('コメント返信の記録が少なめです。交流時間も予定に入れると秘書らしく回せます。');
  if(!notes.length) notes.push('投稿ジャンルの偏りは大きくありません。この調子でやさしく続けられます。');
  return { posts, byCategory: Object.fromEntries(byCategory), videoCount, replies, popular, notes };
}
